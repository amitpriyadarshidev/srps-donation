...
import { Country, Currency, Purpose, FormErrors, DonationFormData, State } from '@/types/donation';
...
export default function DonationFormPage() {
  ...
  const initialCountryId = donation?.country_id || detectedCountry?.id;
  ...
  const calcSymbolWidth = useCallback((symbol: string) => {
    const baseWidth = 20;
    const charWidth = symbol.length * 8;
    const extraWidth = /[₹£€]/.test(symbol) ? 4 : 0;
    return Math.max(baseWidth, charWidth + extraWidth + 16);
  }, []);

  const calcCodeWidth = useCallback((code: string) => {
    const baseWidth = 80;
    const charWidth = 8;
    const padding = 16;
  ...
  const normalizePhoneCode = useCallback((pc?: string) => (pc ? (pc.startsWith('+') ? pc : `+${pc}`) : ''), []);

  const countryOptions: ComboboxOption[] = useMemo(
    () => countries.map(c => ({ value: c.id, label: c.name, extra: c.code })),
    [countries]
  );
  const currencyOptions: ComboboxOption[] = useMemo(
    () => currencies.map(c => ({ value: c.id, label: `${c.name} (${c.code})`, extra: c.symbol })),
    [currencies]
  );
  const purposeOptions: ComboboxOption[] = useMemo(
    () => purposes.map(p => ({ value: p.id, label: p.name, extra: (<div className="flex items-center gap-1"><span>{p.icon}</span><span className="text-xs text-muted-foreground">{p.category}</span></div>) })),
    [purposes]
  );
  const stateOptions: ComboboxOption[] = useMemo(
    () => states.map(s => ({ value: s.id, label: s.name, extra: s.code })),
    [states]
  );
  const phoneCodeOptions: ComboboxOption[] = useMemo(() => {
    const map = new Map<string, string>();
    for (const c of countries) {
      const code = normalizePhoneCode(c.phone_code);
      if (!map.has(code)) {
  ...
  useEffect(() => {
    ...
    const c = currencies.find(x => x.id === formData.currency);
    const sym = c?.symbol || '$';
    setCurrencySymbol(sym);
    setSymbolWidth(calcSymbolWidth(sym));
  }, [formData.currency, currencies, calcSymbolWidth]);

  useEffect(() => {
    if (initialCountryId && !donation?.country?.states && !states.length) {
      loadStates(initialCountryId, donation?.state_id);
    }
  }, [donation, initialCountryId, states.length]);
  ...
  const clearError = useCallback((name: string) => {
    ...
    setLocalErrors(prev => { const ne = { ...prev }; delete ne[name]; return ne; });
  }, [localErrors]);
  ...
  const handleCountryChange = useCallback((id: string) => {
    if (!id) {
      setFormData(p => ({ ...p, country: '', currency: '', phone_country_code: '', state: '' }));
      setStates([]);
      return;
    }
    const c = countries.find(x => x.id === id);
    if (!c) return;
    const phoneCode = normalizePhoneCode(c.phone_code);
    setFormData(p => ({ ...p, country: id, currency: c.default_currency, phone_country_code: phoneCode, state: '' }));
    if (c.states) { setStates(c.states); } else { loadStates(id); }
    clearError('country');
  }, [countries, normalizePhoneCode, clearError]);
  const addDocument = useCallback(() => setFormData(p => ({ ...p, documents: [...p.documents, { type: '', file: null }] })), []);
  const removeDocument = useCallback((i: number) => {
    if (formData.documents.length > 1) {
      setFormData(p => ({ ...p, documents: p.documents.filter((_, idx) => idx !== i) }));
    }
  }, [formData.documents.length]);
  const updateDocument = useCallback((index: number, field: 'type' | 'file', value: any) => {
    setFormData(p => ({ ...p, documents: p.documents.map((d, i) => i === index ? { ...d, [field]: value } : d) }));
    const key = `documents.${index}.${field}`;
    if (localErrors[key]) {
      setLocalErrors(prev => { const ne = { ...prev }; delete ne[key]; if (ne.documents && typeof ne.documents === 'string') delete ne.documents; return ne; });
    }
  }, [localErrors]);
    ...
    Object.entries(formData).forEach(([k, v]) => {
      if (k === 'documents') {
        formData.documents.forEach((doc, i) => {
          ...
          if (doc.file) fd.append(`documents[${i}][file]`, doc.file as File);
        });
    ...
    <DonationLayout title="Donate">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-black/5 p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className={`flex rounded-md ${localErrors.phone || localErrors.phone_country_code || localErrors.phone_combined ? 'ring-1 ring-red-500' : ''}`}>
                    <div className="relative" style={{ minWidth: 'fit-content', width: `${calcCodeWidth(formData.phone_country_code)}px` }}>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex items-center">
                  <input type="checkbox" id="tax_exemption" checked={formData.tax_exemption} onChange={(e) => handleInputChange('tax_exemption', e.target.checked)} className="size-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {formData.documents.length > 1 && (
                            <Button type="button" variant="outline" size="sm" onClick={() => removeDocument(index)} className="flex-shrink-0 size-9 p-0 border-gray-300 hover:border-red-500 hover:bg-red-50 hover:text-red-600 transition-colors">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="flex items-center justify-start md:justify-end">
                  <input type="checkbox" id="skip_kyc" checked={formData.skip_kyc} onChange={(e) => handleInputChange('skip_kyc', e.target.checked)} className="size-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500" />
          ...
          <div>
            <div className="flex items-center">
              <input type="checkbox" id="terms_accepted" checked={formData.terms_accepted} onChange={(e) => handleInputChange('terms_accepted', e.target.checked)} className={`size-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500 ${localErrors.terms_accepted ? 'border-red-500' : ''}`} />
          ...
          <div className="flex justify-center">
            <Button type="submit" disabled={isLoading} size="lg" className="px-8">
              {isLoading ? (<><div className="animate-spin -ml-1 mr-3 size-5 border-2 border-white border-t-transparent rounded-full"></div>Processing...</>) : (<>💳 Pay Now</>)}
            </Button>
          </div>
        </form>
      </div>
